...
local function disableSilo( event )
...
function Silo.onRocketLaunched( event )
  ...
  if pre_place_silo == Config.ALL and global.world.cities[global.world.city_names[1]].rocket_silo then -- ensures a rocket silo exists for any city
  ...
  if pre_place_silo ~= Config.ALL then
    local this_rocket_silo = global.world.rocket_silo
    this_rocket_silo.launches_this_silo = this_rocket_silo.launches_this_silo + 1
  end

  local remaining_launches = rocket_silo.required_launches - rocket_silo.total_launches
  if remaining_launches < 0 then return end
...
function Silo.onResearchFinished(event)
  ...
  if (rocket_silo.required_launches - rocket_silo.total_launches) <= 0 then return end
...
function Silo.onInit()
  ...
  if pre_place_silo == Config.ALL then
    local silo_crafting_count = #global.world.city_names
    global.world.rocket_silo.required_launches = silo_crafting_count
  ...
  if pre_place_silo ~= Config.NONE then
